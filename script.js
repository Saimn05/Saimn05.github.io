...
function setupPageTransitions(){
  document.addEventListener('click', ev=>{
    // Prefer direct anchor or project-card anchor
    let a = ev.target.closest('a.project-card, a:not(.project-card .project-card *)');
    if(!a) return;
...
function setupLightbox(){
  ...
  document.addEventListener('click', ev=>{
    ...
    if(!(figure && figure.closest('.project-media'))) return;
    ev.preventDefault();
